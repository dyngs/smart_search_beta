...
def test_launch_and_update():
    ...
    retriever_test = EmbeddingRetriever(document_store=test_database.document_store,
                                        embedding_model="models/4_eca_retriever_sr_disitlbert-dot-tas_b-b256-msmarco",
                                        model_format="sentence_transformers")
    test_database.update_document_store('data_test_database', retriever_test)
    print(test_database.current_number_of_documents)
    assert test_database.current_number_of_documents == (72+141+86)
